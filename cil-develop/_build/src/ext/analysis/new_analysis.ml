...
let print_result (name, set) =
   (*E.log "\nPrinting pointer analysis result:";*)
   let rec print_set s =
      match s with
          [] -> ()
...
let display_list1 l =
  let f elem = E.log "\tLock name : %s" elem
  in List.iter f l
...
let display_rt l =
  let f elem = Printf.printf "\nTask -- %s ,RT -- %f" elem.tid  elem.c
  in List.iter f l
...
let rule1 (t1 : task) (t2 : task) (tasks : task list) blist  =
  ...
  E.log "\n Task : %s %s -> Priority : %d %d" t1.tname t2.tname t1.priority t2.priority;
  if ((t1.priority = t2.priority) && 
      (not (share_lock tasks blist t1)) && 
      (not (share_lock tasks blist t2)))
  then begin E.log "\n Task : %s %s -> Priority : %d %d...True-> Removed from list"t1.tname t2.tname t1.priority t2.priority;false end
  else begin E.log "...False-> Not Removed from list";true end
  
let rule2 (t1 : task) (t2 : task) (tasks : task list) blist  =
 (* E.log "\nIn Applying rule 2 => %s -> %d , %s -> %d" t1.tname t1.periodicity t2.tname t2.periodicity;*)
  if ((t1.periodicity = t2.periodicity) && 
      (not (share_lock tasks blist t1)) && 
      (not (share_lock tasks blist t2)))
  then false
  else begin E.log " Not removed";true end
...
in
  (*Printf.printf "\n li = %f, new_li = %f" li new_li;*)

  begin
  (*if (li = new_li) then begin Printf.printf "\t\t======================>TRUE";true end*)
...
let rec loop_while li li_prev tsk tasklist blkList =
  (*Printf.printf "\nloop_while - RT - %f, Period - %d" li tsk.periodicity;
  let lower = List.filter (fun x -> (x.pr < tsk.priority)) !blk_response_time
...
in*)
 ...
 begin
   Printf.printf "\nSchedulable2 - %d, task - %s, li - %f, Period - %d" !schedulable tsk.tname li tsk.periodicity ;
...
let rule4 (t1 : task) (t2 : task) (tasks : task list) blist =
  (*E.log "\nIn Applying rule 4 on tasks %s %s" t1.tname t2.tname;*)
  let low, high = get_low_high t1 t2 
in
  (*E.log "\n OUTSIDE IF  Low_period(%s)= %d ; High_period(%s) = %d" 
	  low.tname (low.periodicity)  high.tname (high.periodicity);*)
  if (high_isMultiple_of_low high.periodicity low.periodicity) then
  begin
   (* E.log "hightask is a multiple of low task high:%d low:%d" high.periodicity low.periodicity;*)
    if(not ( share_lock_low tasks blist low high)) then
	begin	
	(*E.log "\n INSIDE IF  period(%s)= %d ; period(%s) = %d" 
	  low.tname (low.periodicity)  high.tname (high.periodicity);*)
  	false
	end
      (*if ((wrt low low.wcet !task_response_time) <= (float_of_int(high.periodicity))) then 
...
let rule5 (t1 : task) (t2 : task) (tasks : task list) blist =
  ...
  let low,high = get_low_high t1 t2 in
	...
	E.log "\n OUTSIDE IF1  period(%s)= %d ; period(%s) = %d" 
	  t1.tname (t1.periodicity)  t2.tname (t2.periodicity);
	if(not (isMultiple t1.periodicity t2.periodicity)) then
          if(not ( share_lock_low tasks blist low high)) then
	    begin
	    E.log "\n OUTSIDE IF2  min_val = %d ; wcrt_low(%s) = %d" 
	    min_val  low.tname (int_of_float(wrt low low.wcet !task_response_time));
  ...
  Set(lv,e,loc) -> 
	...
	Var(vi) ->  
	  ...
	  match off1 with
	    ...
	    | Field(fi,_) -> 
		(*print_string ("\nCheck field1"^vi.vname^"."^fi.fname);*)
		begin 
		if(isin vlist vi) 
		  then 
		  begin 
...
(*let rec analyse rule result taskList tlist blist =
  E.log "debug:in analyse";
  match taskList with
  [] -> E.log "\n...........................Empty";result @ []
  | h::t ->E.log "\n...........................h::t";
      begin
        let task1 = (get_task_by_name (get_1 h) tlist)
	and task2 = (get_task_by_name (get_2 h) tlist)
        in
	  E.log "debug: in analyse filter";
	  let filtered_ctp = List.filter (fun x -> rule task1 task2 tlist blist) taskList in
	  result @ filtered_ctp;
	  analyse rule result t tlist blist
      end  *)
...
let rec presentIn accessPair taskFunPair =
 match taskFunPair with
  [] -> (*E.log "\nDEBUG:presentIn2\t ";*)false
  | hd :: tl -> (*E.log "\nDEBUG:presentIn3";*)
     match hd with 
	((t1,t2),(f1,f2)) -> (*E.log "\nDEBUG:presentIn4";*)
	  match accessPair with
	     ((fn,(ln,var,w,r)),(fn',(ln',var',w',r'))) -> (*E.log "\nDEBUG:presentIn5";*)
...
(*let presentIn accessPair taskFunPair =
  E.log "\n taskFunPair length : %d" (length taskFunPair);
 if ((length taskFunPair) = 0) then begin E.log "\n==========>Empty list";true end
 else 
    presentIn1 accessPair taskFunPair*)
...
let maindb (f : file) =
  ...
  E.log "\nPtr Analysis CIL %d"(length !pa_result);
  ...
  E.log "\n**********************CTPs**********************%d" (length conflictTaskPair);
