...
type blk_rt = { tid : string; lid : string; c : float; pr : int; }
...
val print_result : Cil.varinfo * Cil.varinfo list -> unit
val display_list1 : string list -> unit
val display_list : task list -> unit
val display_cAPair_list : (string * string list) list -> unit
val display_blkList : blk_rt list -> unit
val display_rt : blk_rt list -> unit
...
val display_taskpair : (task * task) list -> unit
val display_funpair : (Cil.fundec * Cil.fundec) list -> unit
val display_tfpairlist : (task * Cil.fundec) list -> unit
...
val displayTaskFun : (task * Cil.fundec) list -> unit
...
val computeL_i : float -> task -> task list -> float
val loop_while : float -> task -> task list -> lock_block list -> float
...
val isin_g : Cil.varinfo list -> string -> bool
val issame : 'a list -> 'a -> bool
val issame_var : Cil.varinfo list -> string -> bool
...
val contain :
  Cil.instr -> Cil.varinfo list -> 'a -> (int * string * bool * bool) list
val findCaseVar : Cil.stmt list -> string list
val isPresent_stmt :
  Cil.stmt ->
  Cil.varinfo list -> 'a -> (int * string * bool * bool) list list
val findstmts :
  Cil.varinfo list ->
  Cil.fundec -> 'a -> (int * string * bool * bool) list list list
...
val getTaskFun : task list -> Cil.fundec list -> (task * Cil.fundec) list
